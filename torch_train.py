...
def main():
    ...
    if args.load_gpu:
        ...
        if not args.mpath or (args.mpath and args.continue_training):
            train_loader = DataLoader(data_train, batch_size=args.batchsize,shuffle=True,)#multiprocessing_context='spawn',num_workers=16)#,pin_memory=True)
        val_loader = DataLoader(data_val, batch_size=args.batchsize,shuffle=True,)#multiprocessing_context='spawn',num_workers=16)#,pin_memory=True)
        ...
        if not args.mpath or (args.mpath and args.continue_training):
            train_loader = DataLoader(data_train, batch_size=args.batchsize,shuffle=False,multiprocessing_context='spawn',num_workers=16)#,pin_memory=True)
        val_loader = DataLoader(data_val, batch_size=args.batchsize,shuffle=False,multiprocessing_context='spawn',num_workers=16)#,pin_memory=True)
    ...
    #    backend='nccl', world_size=4, init_method='...'
    #)
    ...
    if args.mpath:
        if args.continue_training:
            ...
            #print(models)
            #sys.exit(1)
            model = train_classifier(model, loss, batchsize, n_epochs, model.module.name, "/".join(args.mpath.split("/")[:-1]),
            )
        else:
            run_inference(args.mpath, args.plot_text, model.module.name, args.mpath+"_plots", 
        ...
        model = train_classifier(model, loss, batchsize, n_epochs, model.module.name, outdir+"/models/", 
        ...
        eval_classifier(model, args.plot_text, model.module.name, outdir+"/plots/", )#particleDataTest, labelsTest, singletonDataTest, svTestingData=vertexDataTest, eventTestingData=singletonFeatureDataTest,pfMaskTestingData=maskpfTest,svMaskTestingData=masksvTest,) 
